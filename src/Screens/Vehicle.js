...
import { View, Text,Dimensions,StyleSheet,TextInput, ScrollView,TouchableOpacity,Image} from "react-native";
...
writeUserData(  
    ...
    userID=uid;
    firebase.database().ref('Users/'+userID+'/UserProducts/Vehicles/').push({
    ...
    //   firebase.database().ref('User/'+userID+'/UserProducts/MobilePhones/'+productID).update({
     var uid = firebase.auth().currentUser.uid;
     firebase.database().ref().child('Users').child(uid).child('UserProducts').child('Vehicles').child(productID).update({
    productID
    });
  }).catch((error) => {
 ...
 render() {
        ...
        <View style={{alignItems:'center',justifyContent:'center',backgroundColor:'white'}} >
          <View style={styles.desStyle}>
            ...
            <View style={styles.viewStyle}>
               <Text style={styles.textStyle} > KM Driven: </Text>
                   <TextInput style={styles.inputBox} 
                    placeholder="7500" 
                    placeHolderTextColor= "black"
                    value={this.state.km}
