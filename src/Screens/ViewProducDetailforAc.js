...
export default class ViewAcDetails extends Component {
...
*/
    render(){
        ...
        const chosenDate =navigation.getParam('chosenDate','null');
        const StartingBid =navigation.getParam('StartingBid','null');
        const TonSelected =navigation.getParam('TonSelected','null');
        const TypeSelected =navigation.getParam('TypeSelected','null');
        ...
        <ScrollView style={{padding:2}}>
           <View style={styles.viewStyle}>
              ...
              <View style={{borderColor:'white',borderWidth:2,marginTop:20,height:160,padding:8,shadowColor: '#fff',elevation:3,shadowOpacity:1,shadowColor:'grey'}}>
                    <Text  style={{fontSize:19,fontWeight:'bold', fontFamily: "Times New Roman",marginTop:5}}>BRAND: {JSON.stringify(Brand).replace(/\"/g, "")} </Text>
                    <Text  style={{fontSize:19,fontWeight:'bold', fontFamily: "Times New Roman",marginTop:5}}>MODEL: {JSON.stringify(Model).replace(/\"/g, "")}</Text>
                    <Text  style={{fontSize:19,fontWeight:'bold', fontFamily: "Times New Roman",marginTop:5}}>CONDITION: {JSON.stringify(Condition).replace(/\"/g, "")}</Text>
                    <Text  style={{fontSize:19,fontWeight:'bold', fontFamily: "Times New Roman",marginTop:5}}>TonSelected: {JSON.stringify(TonSelected).replace(/\"/g, "")}</Text>
                    <Text  style={{fontSize:19,fontWeight:'bold', fontFamily: "Times New Roman",marginTop:5}}>TypeSelected: {JSON.stringify(TypeSelected).replace(/\"/g, "")}</Text>
                </View>
                ...
                <View style={{borderColor:'white',borderWidth:2,marginTop:20,height:170,padding:8,shadowColor: '#fff',elevation:3,shadowOpacity:1,shadowColor:'grey'}}>
                    <Text style={{fontSize:19,fontWeight:'bold', fontFamily: "Times New Roman"}}>FLOOR:{JSON.stringify(chosenDate).replace(/\"/g, "")}</Text>
                    <Text style={{fontSize:19,fontWeight:'bold', fontFamily: "Times New Roman"}}>FLOOR:{JSON.stringify(Description).replace(/\"/g, "")}</Text>
